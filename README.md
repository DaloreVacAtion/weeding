...
### 9. Внесение правок и редеплой

Когда вы изменили код, тексты, картинки или другой контент:

**Вариант А — через Git**

1. Локально: внесите изменения, закоммитьте и отправьте в репозиторий:
   ```bash
   git add .
...
2. На сервере: подтяните изменения и пересоберите:
   ```bash
   cd wedding-invitation
   git pull
   docker compose up -d --build
...
**Вариант Б — без Git (копирование файлов)**

1. Скопируйте изменённые файлы на сервер (через SCP, SFTP, FileZilla и т.п.).
...
**Что важно:**
- Миграции выполняются при каждом запуске (`migrate --noinput` в команде).
- `collectstatic` выполняется при каждом запуске — новые картинки и статика подхватятся.
- База и тома (`postgres_data` и др.) сохраняются при `--build`.
- Суперпользователь создаётся только один раз, повторно его создавать не нужно.
